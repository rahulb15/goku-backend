(namespace "free")
(define-keyset "free.testdbone-ks" (read-keyset "testdbone-ks"))

(module testdbcontract1 COOPER
  ...
  (defcap COOPER ()
    (enforce-guard (keyset-ref-guard "free.testdbone-ks")))
  ...
  (defun TRANSFER-mgr:decimal
    ...
    (defun create-account:bool
      ...
      ;   (insert collections-table (at 'name collection)
      ;     {
        ...
        (with-read collections-table name
          ...
          (enforce-one "Only Admin or Collection creator can create tokens-table"
            [(enforce-guard collection-guard)
             (enforce-guard (keyset-ref-guard "testdbone-ks"))])
        )
      )
      ...
      (defun credit:object{receiver-balance-change}
        ...
        (enforce-one "Capabilities not installed."
          [(require-capability (CREDIT id account))
           (require-capability (CREDIT-COOPER account))])
