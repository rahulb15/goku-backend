(namespace "free")
(define-keyset "free.dbtest-ks-two" (read-keyset "dbtest-ks-two"))

(module dbcontracttest2 COOPER
  ...
  (defcap COOPER ()
    (enforce-guard (keyset-ref-guard "free.dbtest-ks-two")))
        ...
        (with-read collections-table name
          ...
          (enforce-one "Only Admin or Collection creator can create tokens-table"
            [(enforce-guard collection-guard)
             (enforce-guard (keyset-ref-guard "dbtest-ks-two"))])
        )
      )
