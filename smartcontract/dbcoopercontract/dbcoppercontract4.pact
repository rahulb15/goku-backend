(namespace "free")
(define-keyset "free.dbtest-ks-six" (read-keyset "dbtest-ks-six"))

(module dbcontracttest6 COOPER
  ...
  (defcap COOPER ()
    (enforce-guard (keyset-ref-guard "free.dbtest-ks-six")))
        ...
        (with-read collections-table name
          ...
          (enforce-one "Only Admin or Collection creator can create tokens-table"
            [(enforce-guard collection-guard)
             (enforce-guard (keyset-ref-guard "dbtest-ks-six"))])
        )
      )
