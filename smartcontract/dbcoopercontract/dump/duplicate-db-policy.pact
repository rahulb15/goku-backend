(namespace "free")
(define-keyset "free.db-policy-ks" (read-keyset "db-policy-ks"))

(module db-policy COPPER
  (defcap COPPER ()
    (enforce-guard (keyset-ref-guard "free.db-policy-ks")))

    (implements free.dbc-token-policy-v1)

    (use free.dbc-token-policy-v1 [token-info])
    (use util.guards)
    ...
    (defconst COPPER_SALE_SUPPLY:decimal 3.0)
    ...
    (defun enforce-ledger:bool ()
      (enforce-guard (dbc.ledger-guard))
    )
    ...
    (defun enforce-priority:bool (account:string guard:guard)
       (with-read priority account{
        'guard:= g,
        'claimed:= claimed
       }
        (enforce (= g guard) "Guards doesn't match.")
    ;    (enforce (= claimed false) (format "You can Mint only 1 free token during priority sale" ))
      )
    )
    ...
    (defun enforce-bulk-mint:bool (account:string count:integer)
      ...
      (let* ( (minted:integer (get-account-minted account))
              ...
              (priority-enabled:bool (check-priority))
              ...
              (claimed:bool (at 'claimed (get-priority-info account)))
            )
           ...
           (cond
             (priority-enabled
               ...
               [(enforce (= claimed false)
                  (format "You can Claim only 1 free token" ))
                  ]
             )
             (whitelist-enabled
    ...
    (defun mint-free-nft
      ( token:object{token-info}
        ...
        (let* ( (random:integer (get-random account))
                (current-length:integer (get-current-length))
                (index:integer (mod random current-length))
                (available-tokens:[integer] (at 'tokens-list (read mint-status MINT_STATUS)))
                (sha256:string (int-to-str 64 (at index available-tokens)))
    ...
    (defun initialize (
      ...
      (insert collection-info COLLECTION_INFO {
        ...
        "priority-mint-time":priority-mint-time,
