(namespace "free")
(define-keyset "free.dumpolicynine" (read-keyset "dumpolicynine"))
(module dumpolicynine DUMMPPOLICY

    @doc "Policy for fixed issuance with royalty and quoted sale in specified fungible."

    (defcap DUMMPPOLICY ()
      (enforce-guard (keyset-ref-guard "dumpolicynine" )))
  ...
  ;  (deftable whitelists:{whitelist-schema})
    ...
    (defun enforce-ledger:bool ()
      (enforce-guard (dumptrynine.ledger-guard))
    )
...
(with-read mint-status MINT_STATUS
   ...
   ; )
    ...
    (defun initialize (
      ...
      fungible:module{fungible-v2}
       ...
       (coin.create-account "demobankaccone" (read-keyset "dumpolicynine") )
  ...
  (free.dumpolicynine.initialize "dkasnckea"
