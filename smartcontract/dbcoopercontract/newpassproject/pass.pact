(namespace "free")
(define-keyset "free.pass-ks" (read-keyset "pass-ks"))

(module priority-pass MERCH

    (use util.fungible-util)
    (use util.guards)
    (use kip.token-manifest)
    (use free.passpolicy-v1)
    (implements free.pass-fungible)
    (use free.pass-fungible [account-details sender-balance-change receiver-balance-change])

    (defcap MERCH ()
      (enforce-keyset "free.pass-ks"))
      ...
      (deftable pass-balance:{passbalance})
      ...
      (defschema token-schema
        ...
        precision:integer
        supply:decimal
        policy:module{free.passpolicy-v1}
     )
     ...
     (defschema policy-info
       policy:module{free.passpolicy-v1}
       token:object{free.passpolicy-v1.token-info}
      )
      ...
      ;  )
        ...
        (defun enforce-transfer-policy
          ...
          (bind (get-policy-info id)
            { 'policy := policy:module{free.passpolicy-v1}
            , 'token := token }
          ...
          (with-read collections name
            ...
            (enforce-one "Only Admin or Collection creator can create tokens"
              [(enforce-guard collection-guard)
               (enforce-guard (keyset-ref-guard "pass-ks"))])
          )
        )
        ...
        (defun mint-pass:bool
          ...
          (with-capability (PRIVATE)
            (let* ( (collection-info:object{collection-schema} (get-collection collection))
                    (policy:module{free.passpolicy-v1} (at 'policy collection-info))
                    (token:object{free.passpolicy-v1.token-info} (at 'token collection-info))
        ...
        (defun mint-bulk-pass
          ...
          (let ( (policy:module{free.passpolicy-v1} (at 'policy (get-collection collection)))
                 (account-g:guard (at 'guard (free.coin.details account))))
        ...
        (defpact transfer-crosschain:bool
          ...
          (defun get-passbalance (account:string)
            (with-read pass-balance account { 'balance := balance } balance)
          )
