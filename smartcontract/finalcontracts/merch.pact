(namespace "free")
(define-keyset "free.merchfinalks001" (read-keyset "merchfinalks001"))

(module merchfinal001 MERCH
  ...
  (defcap MERCH ()
    (enforce-guard (keyset-ref-guard "free.merchfinalks001"))
  ...
  (defun TRANSFER-mgr:decimal
    ...
    (defun create-account:bool
      ...
      (defun launch-nft-collection:bool
        ...
        (free.merchfinalpolicy001.create-nft-collection tokens-list creator guard total-supply 1 price name imageUrl)
        ...
        (with-read collections-table name
          ...
          (enforce-one "Only Admin or Collection creator can create tokens-table"
            [(enforce-guard collection-guard)
             (enforce-guard (keyset-ref-guard "merchfinalks001"))])
        )
      )
    ...
    (defun update-price:string (price:decimal collectionName:string)
      ...
      (enforce (< 0.0 price) "price is not a positive number")
        ...
        (free.merchfinalpolicy001.update-mint-price price collectionName)
      ...
      (defun mint:bool
        ...
        (let*
              (
                ...
                (imgUrl:string (format "{}/{}" [imageUrl tkn-id]))
                (datum-object
                  {
                    'creator: creator
