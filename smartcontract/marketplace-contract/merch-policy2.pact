(namespace "free")
(define-keyset "free.mpolicyseven-ks" (read-keyset "mpolicyseven-ks"))
(module mpolicy7 MERCH

  @doc "Policy for fixed issuance with royalty and quoted sale in specified fungible."

  (defcap MERCH ()
    (enforce-guard (keyset-ref-guard "free.mpolicyseven-ks")))
...
(defcap BUY (id:string receiver:string)
 (compose-capability (UPDATE-OWNER id receiver))
  ...
  (defun enforce-ledger:bool ()
    (enforce-guard (mtest5.ledger-guard))
  )
  ...
  (defun initialize (
    ...
    name:string
    fungible:module{fungible-v2})
    (coin.create-account "merchcreator7" (read-keyset "mpolicyseven-ks"))
    (coin.create-account "racc7" (read-keyset "mpolicyseven-ks"))
