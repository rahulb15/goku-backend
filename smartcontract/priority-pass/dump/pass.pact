(namespace 'free)
(define-keyset "free.dum-pass-ks" (read-keyset "dum-pass-ks"))

(module dumptryfour MERCH
    ...
    (defcap MERCH ()
      (enforce-guard (keyset-ref-guard "free.dum-pass-ks")))

    ; (defcap MERCH ()
    ;   (enforce-keyset "dum-pass-ks"))
          ...
          (with-read collections name
            ...
            (enforce-one "Only Admin or Collection creator can create tokens"
              [(enforce-guard collection-guard)
               (enforce-guard (keyset-ref-guard "dum-pass-ks"))])
          )
        )
