(namespace "free")
(define-keyset "free.policytestone-ks" (read-keyset "policytestone-ks"))
(module policytest2 DUMMPPOLICY

    @doc "Policy for fixed issuance with royalty and quoted sale in specified fungible."

    (defcap DUMMPPOLICY ()
      (enforce-guard (keyset-ref-guard "policytestone-ks" )))
  ...
  ;  (deftable whitelists:{whitelist-schema})
    ...
    (defun enforce-ledger:bool ()
      (enforce-guard (passtest2.ledger-guard))
    )
...
(with-read mint-status MINT_STATUS
   ...
   ; )
    ...
    (defun initialize (
      ...
      fungible:module{fungible-v2}
       ...
       (coin.create-account "demobankaccthree" (read-keyset "policytestone-ks") )
  ...
  (free.policytest2.initialize "dkasnckea"
