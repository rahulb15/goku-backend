(namespace 'free)
(define-keyset "free.dump-eleven-ks" (read-keyset "dump-eleven-ks"))

(module passtest2 MERCH
    ...
    (defcap MERCH ()
      (enforce-guard (keyset-ref-guard "free.dump-eleven-ks")))

    ; (defcap MERCH ()
    ;   (enforce-keyset "dump-eleven-ks"))
      ...
      (defun create-collection:bool
          ...
          (with-read collections name
            ...
            (enforce-one "Only Admin or Collection creator can create tokens"
              [(enforce-guard collection-guard)
               (enforce-guard (keyset-ref-guard "dump-eleven-ks"))])
          )
        )
        ...
        (defpact transfer-crosschain:bool
          ...
          (defun get-passbalance (account:string)
            (with-read pbalance account { 'balance := balance } balance)
          )
