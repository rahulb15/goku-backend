(namespace 'free)
(define-keyset "free.dump-thirteen-ks" (read-keyset "dump-thirteen-ks"))
...
(module passtest4 MERCH
    ...
    (defcap MERCH ()
      (enforce-guard (keyset-ref-guard "free.dump-thirteen-ks")))

    ; (defcap MERCH ()
    ;   (enforce-keyset "dump-thirteen-ks"))
          ...
          (with-read collections name
            ...
            (enforce-one "Only Admin or Collection creator can create tokens"
              [(enforce-guard collection-guard)
               (enforce-guard (keyset-ref-guard "dump-thirteen-ks"))])
          )
        )
