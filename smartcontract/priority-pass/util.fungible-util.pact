(namespace 'util)
(define-keyset "util.util-ks" (read-keyset "util-ks"))
(module fungible-util MERCH
  (implements kip.account-protocols-v1)

  (defcap MERCH ()
    (enforce-guard (keyset-ref-guard "util-ks")))
    ...
    true
      (let ((r (check-reserved account)))
        (if (= r "")
          true
          (if (= r "k")
            (enforce true "Single-key account protocol violation")
            (enforce false
